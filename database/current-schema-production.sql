...
-- Generated: 2025-07-22T22:33:14.836Z
...
-- Table: emails
CREATE TABLE public.emails (id UUID NOT NULL DEFAULT uuid_generate_v4(), account_id UUID NOT NULL, message_id VARCHAR(255) NOT NULL, thread_id VARCHAR(255), subject TEXT, body_text TEXT, body_html TEXT, sender_email VARCHAR(255), sender_name VARCHAR(255), recipient_emails ARRAY, cc_emails ARRAY, bcc_emails ARRAY, received_at TIMESTAMP WITH TIME ZONE, sent_at TIMESTAMP WITH TIME ZONE, is_processed BOOLEAN DEFAULT false, processing_status processing_status DEFAULT 'pending'::processing_status, processing_error TEXT, created_at TIMESTAMP WITH TIME ZONE DEFAULT now(), updated_at TIMESTAMP WITH TIME ZONE DEFAULT now(), processed_for_faq BOOLEAN NOT NULL DEFAULT false, processed_at TIMESTAMP WITH TIME ZONE);

-- Table: faq_groups
CREATE TABLE public.faq_groups (id UUID NOT NULL DEFAULT uuid_generate_v4(), title VARCHAR(500) NOT NULL, representative_question TEXT NOT NULL, consolidated_answer TEXT NOT NULL, question_count INTEGER DEFAULT 0, frequency_score DOUBLE PRECISION DEFAULT 0, avg_confidence DOUBLE PRECISION DEFAULT 0, representative_embedding vector, is_published BOOLEAN DEFAULT false, category VARCHAR(100), tags ARRAY, created_at TIMESTAMP WITH TIME ZONE DEFAULT now(), updated_at TIMESTAMP WITH TIME ZONE DEFAULT now());
...
ALTER TABLE emails ADD CONSTRAINT emails_account_id_message_id_key UNIQUE (account_id, account_id, message_id, message_id);
...
ALTER TABLE question_groups ADD CONSTRAINT question_groups_pkey PRIMARY KEY (question_id, question_id, group_id, group_id);
...
ALTER TABLE questions ADD CONSTRAINT questions_email_question_unique UNIQUE (email_id, email_id, question_text, question_text);
